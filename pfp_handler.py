    ...
    try:
        ...
        if "facebook.com" in parsed_url.netloc:
            ...
            return {
                "status": "error",
                "error":
                "❌ শুধুমাত্র ফেসবুক বা ইনস্টাগ্রাম প্রোফাইলের URL দিন।"
            }
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url) as res:
                ...
                if 'image' in content_type:
                    ...
                    return {"status": "success", "image_path": temp_path}
                else:
                    data = await res.json()
                    return {
                        "status":
                        ...
                        "error":
                        data.get("error",
                                 "❌ API থেকে কোনো তথ্য পাওয়া যায়নি।")
                    }

    except aiohttp.ClientResponseError as e:
        return {
            "status": "error",
            "error": f"API ত্রুটি: {e.status} - {e.message}"
        }
    except Exception as e:
        return {
            "status": "error",
            "error": f"একটি অপ্রত্যাশিত ত্রুটি হয়েছে: {str(e)}"
        }


def register(bot: telebot.TeleBot, custom_command_handler,
    ...
    def handle_pfp_command(message):
        ...
        for prefix in command_prefixes_list:
            if command_text.startswith(
                    f"{prefix}pfp") or command_text.startswith(
                        f"{prefix}profilepic"):
        ...
        thinking_message = bot.reply_to(
            message, "⏳ প্রোফাইল ছবিটি ডাউনলোড করা হচ্ছে...")
        ...
        try:
            ...
            if result['status'] == 'success':
                ...
                bot.delete_message(thinking_message.chat.id,
                                   thinking_message.message_id)
            else:
                bot.edit_message_text(chat_id=thinking_message.chat.id,
